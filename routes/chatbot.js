...
async function handleChatbotQuery(message) {
  ...
  try {
    ...
    if (
      ...
      (lowerMessage.includes("district") || lowerMessage.includes("city") || 
       lowerMessage.includes("area") || lowerMessage.includes("hyderabad") || 
       lowerMessage.includes("bhubaneswar") || lowerMessage.includes("cuttack") ||
       lowerMessage.includes("puri") || lowerMessage.includes("berhampur"))
    ) {
      // Extract location name
      let location = "";
      const locationKeywords = ["hyderabad", "bhubaneswar", "cuttack", "puri", "berhampur", "khorda", "ganjam", "balasore", "sambalpur"];
      ...
      if (location) {
        ...
        if (aadharNumbers.length > 0) {
          ...
          casesFromLocation.forEach(c => {
            ...
            fraudTypeBreakdown[c.typeOfFraud] = (fraudTypeBreakdown[c.typeOfFraud] || 0) + 1;
          });
          ...
          return {
            ...
            links: [
              { text: `View ${location} Cases`, url: "/complaints" }
            ]
          };
        } else {
          return {
        ...
        if (user) {
          ...
          if (userCases.length > 0) {
            ...
            userCases.slice(0, 3).forEach((c, idx) => {
              responseMsg += `${idx + 1}. ${c.caseId} - ${c.typeOfFraud} (${c.status})\n`;
            });
          }
          ...
          return {
            ...
            links: [
              { text: "View All User Cases", url: "/complaints" }
            ]
          };
        } else {
          return {
    ...
    const fraudTypes = [
      "upi", "investment", "customer care", "apk", "job", "debit card", 
      "credit card", "e-commerce", "loan", "sextortion", "olx", "lottery",
      "hotel booking", "gaming", "wallet", "digital arrest", "fake website",
      "ticket booking", "insurance", "facebook", "instagram", "twitter",
      "whatsapp", "telegram", "gmail", "youtube", "fraud call"
    ];
    ...
    for (const fraudType of fraudTypes) {
      if (lowerMessage.includes(fraudType)) {
        ...
        if (matchingCases.length > 0) {
          ...
          let responseMsg = `${fraudType.toUpperCase()} Fraud Cases: ${matchingCases.length}\n\n`;
          responseMsg += `Status Breakdown:\n`;
          Object.entries(statusBreakdown).forEach(([status, count]) => {
            responseMsg += `- ${status}: ${count}\n`;
          });
          ...
          matchingCases.slice(0, 5).forEach((c, idx) => {
            ...
            responseMsg += `${idx + 1}. ${c.caseId} - ${c.status} (${daysOld} days old)\n`;
          });
          ...
          return {
            ...
            links: [
              { text: `View ${fraudType} Cases`, url: "/complaints" }
            ]
          };
        }
      }
    ...
    // Date range queries
    if (
      (lowerMessage.includes("last") && (lowerMessage.includes("days") || lowerMessage.includes("week") || lowerMessage.includes("month"))) ||
      ...
      if (lastWeeksMatch) {
        ...
        dateFilter = { createdAt: { $gte: new Date(Date.now() - weeks * 7 * 24 * 60 * 60 * 1000) } };
      ...
      if (Object.keys(dateFilter).length > 0) {
        ...
        return {
          ...
          links: [
            { text: "View These Cases", url: "/complaints" }
          ]
        };
      }
    }
    ...
    if (
      ...
      if (lowerMessage.includes("unassigned") || lowerMessage.includes("not assigned")) {
        ...
        return {
          ...
          links: [
            { text: "View Unassigned Cases", url: "/complaints" }
          ]
        };
      } else {
        ...
        assignedCases.forEach(c => {
          ...
          officerCaseCount[officerName] = (officerCaseCount[officerName] || 0) + 1;
        });
        ...
        return {
          message: responseMsg,
          data: { assigned: assignedCases.length, officerBreakdown: officerCaseCount },
          links: [
            { text: "View Assigned Cases", url: "/complaints" }
          ]
        };
      }
    }
    ...
    if (
      (lowerMessage.includes("urgent") || lowerMessage.includes("high priority")) &&
      ...
      if (urgentPendingCases.length > 0) {
        ...
        urgentPendingCases.slice(0, 5).forEach((c, idx) => {
          const daysOld = Math.floor((Date.now() - new Date(c.createdAt).getTime()) / (1000 * 60 * 60 * 24));
          responseMsg += `${idx + 1}. ${c.caseId} - ${c.typeOfFraud} (${daysOld} days old)\n`;
        });
      }
      
      return {
        message: responseMsg,
        data: { urgentPending: urgentPendingCases.length, cases: urgentPendingCases },
        links: [
          { text: "View Urgent Pending", url: "/complaints?priority=urgent&status=pending" }
        ]
      };
    }
    ...
    if (
      ...
      lowerMessage.includes("which") && (lowerMessage.includes("area") || lowerMessage.includes("district"))
    ) {
      ...
      for (const user of allUsers) {
        ...
        const userCaseCount = await Cases.countDocuments({ aadharNumber: user.aadharNumber });
        locationCaseCount[location] = (locationCaseCount[location] || 0) + userCaseCount;
      }
    ...
    if (
      (lowerMessage.includes("male") || lowerMessage.includes("female") || lowerMessage.includes("gender")) &&
      (lowerMessage.includes("case") || lowerMessage.includes("complaint") || lowerMessage.includes("how many"))
    ) {
    ...
    // Age-based queries
    if (
      (lowerMessage.includes("age") || lowerMessage.includes("young") || lowerMessage.includes("old") || 
       lowerMessage.includes("senior") || lowerMessage.includes("youth")) &&
      (lowerMessage.includes("case") || lowerMessage.includes("complaint") || lowerMessage.includes("how many"))
    ) {
      const allUsers = await Users.find();
      const ageGroups = {
      ...
      allUsers.forEach(user => {
        if (user.dob) {
          ...
          if (age >= 18 && age <= 25) ageGroups["18-25"].users.push(user.aadharNumber);
          else if (age >= 26 && age <= 35) ageGroups["26-35"].users.push(user.aadharNumber);
          else if (age >= 36 && age <= 45) ageGroups["36-45"].users.push(user.aadharNumber);
          else if (age >= 46 && age <= 60) ageGroups["46-60"].users.push(user.aadharNumber);
      ...
      Object.entries(ageGroups).forEach(([group, data]) => {
        if (data.users.length > 0) {
          ...
          responseMsg += `- Cases: ${data.cases}\n`;
          responseMsg += `- Avg cases per user: ${(data.cases / data.users.length).toFixed(2)}\n\n`;
        }
      });
      ...
      const mostVulnerable = Object.entries(ageGroups)
        ...
        .sort((a, b) => (b[1].cases / b[1].users.length) - (a[1].cases / a[1].users.length))[0];
    ...
    if (
      (lowerMessage.includes("phone") || lowerMessage.includes("mobile") || lowerMessage.includes("number")) &&
      /\d{10}/.test(message) &&
      ...
      if (phoneMatch) {
        ...
        if (user) {
          ...
          return {
            ...
            data: { user, cases: userCases },
            links: [
              { text: "View User Cases", url: "/complaints" }
            ]
          };
        } else {
          return {
    ...
    if (
      ...
      (lowerMessage.includes("show") || lowerMessage.includes("details") || lowerMessage.includes("status"))
    ) {
      ...
      if (caseIdMatch) {
        ...
        const specificCase = await Cases.findOne({ caseId: new RegExp(caseId, "i") });
        
        if (specificCase) {
          const user = await Users.findOne({ aadharNumber: specificCase.aadharNumber });
          ...
          const daysOld = Math.floor((Date.now() - new Date(specificCase.createdAt).getTime()) / (1000 * 60 * 60 * 24));
          ...
          return {
            ...
            links: [
              { text: "View Case Details", url: `/complaints/${specificCase._id}` }
            ]
          };
        } else {
          return {
    ...
    if (
      ...
      lowerMessage.includes("station") && (lowerMessage.includes("case") || lowerMessage.includes("complaint"))
    ) {
      const allUsers = await Users.find();
      ...
      for (const user of allUsers) {
        ...
        if (station && station !== "TBD") {
          const userCaseCount = await Cases.countDocuments({ aadharNumber: user.aadharNumber });
          stationCaseCount[station] = (stationCaseCount[station] || 0) + userCaseCount;
        }
      }
    ...
    if (
      ...
      lowerMessage.includes("new") && (lowerMessage.includes("case") || lowerMessage.includes("complaint"))
    ) {
      const recentCases = await Cases.find()
        .sort({ createdAt: -1 })
        .limit(10);
    ...
    if (
      (lowerMessage.includes("solved") || lowerMessage.includes("resolved") || lowerMessage.includes("closed")) &&
      (lowerMessage.includes("complaint") || lowerMessage.includes("case") || lowerMessage.includes("how many"))
    ) {
      const percentage = ((stats.byStatus.solved / stats.total) * 100).toFixed(1);
      return {
    ...
    if (
      (lowerMessage.includes("pending") || lowerMessage.includes("waiting") || lowerMessage.includes("open")) &&
      (lowerMessage.includes("complaint") || lowerMessage.includes("case") || lowerMessage.includes("how many"))
    ) {
      return {
        ...
        links: [
          ...
          { text: "View Long Pending", url: "/complaints?status=pending&days=30" }
        ]
      };
    }
    ...
    if (
      ...
      (lowerMessage.includes("pending") || lowerMessage.includes("waiting") || lowerMessage.includes("old"))
    ) {
      ...
      if (longPendingCases.length > 0) {
        ...
        longPendingCases.forEach((c, idx) => {
          const daysOld = Math.floor((Date.now() - new Date(c.createdAt).getTime()) / (1000 * 60 * 60 * 24));
          casesList += `${idx + 1}. ${c.caseId} - ${c.typeOfFraud} (${daysOld} days old)\n`;
        });
      }
      ...
      return {
        ...
        links: [
          { text: "View All Long Pending", url: "/complaints?status=pending&days=30" }
        ]
      };
    }
    ...
    if (
      ...
      (lowerMessage.includes("complaint") || lowerMessage.includes("case") || lowerMessage.includes("under"))
    ) {
      return {
        ...
        links: [
          { text: "View Cases Under Review", url: "/complaints?status=under_review" }
        ]
      };
    }
    ...
    if (
      ...
      return {
        ...
        links: [
          { text: "View Investigating Cases", url: "/complaints?status=investigating" }
        ]
      };
    }
    ...
    if (
      ...
      return {
        ...
        links: [
          { text: "View Today's Cases", url: "/complaints?date=today" }
        ]
      };
    }
    ...
    if (
      ...
      return {
        ...
        links: [
          { text: "View Financial Frauds", url: "/complaints?category=Financial" },
          { text: "View Social Media Frauds", url: "/complaints?category=Social" },
          { text: "View Analytics", url: "/analytics" }
        ]
      };
    }
    ...
    if (
      ...
      (lowerMessage.includes("check") || lowerMessage.includes("track") || lowerMessage.includes("my"))
    ) {
      return {
        ...
        data: null,
        links: [
          { text: "Check Status", url: "/complaints" }
        ]
      };
    }
    ...
    if (
      ...
      return {
        ...
        data: null,
        links: [
          { text: "Contact Info", url: "/settings" }
        ]
      };
    }
    ...
    return {
      ...
      data: null,
      links: [
        { text: "Contact Support", url: "/settings" }
      ]
    };
  }
}
